...
// 		$result = $this->db->get_where('news n',array('n.status'=>1,'ni.status'=>1,'n.publish'=>1))->result_array();
	...
	function all_menus(){
	    ...
	    $result = $this->db->query("Select * From ((SELECT mi.*,m.id,m.page_id,m.p_id,m.cms_url,m.external_link,m.sort FROM `menu` m
            ...
            UNION
            (SELECT mi.*,m.id,m.page_id,m.p_id,m.cms_url,m.external_link,m.sort FROM `menu` m
            JOIN menu_item mi ON m.id = mi.menu_id
            WHERE m.status = 1 AND mi.status = 1 AND mi.lang_id = ".$l_id.")) t1 order by t1.sort")->result_array();
	    //print_r($this->db->last_query());die;
    ...
    public function exportAgriReg($cond)
    {
    	 $result = $this->db->query("SELECT IFNULL(cf.name,'NA') as c_name,
									IFNULL(cf.email,'NA') as c_email,
									IFNULL(cf.phone,'NA') as c_phone,
									IFNULL(cf.area,'NA') as c_area,
									IFNULL(cf.message,'NA') as c_message,
									IFNULL(cf.created,'NA') as c_created
									from contact_form_info cf
									where $cond
									order by cf.id")->result_array();
									    	  return $result;
    }

      public function export_trade($condit)
    {
    	 $result = $this->db->query("SELECT IFNULL(ar.state,'NA') as r_state,
    	 									IFNULL(ar.apmc,'NA') as r_apmc,

    	 									 IFNULL(ar.commodity,'NA') as r_commodity,
    	 									IFNULL(ar.min_price,'NA') as r_min_price,

    	 									 IFNULL(ar.modal_price,'NA') as r_modal_price,
    	 									IFNULL(ar.max_price,'NA') as r_max_price,

    	 									 IFNULL(ar.commodity_arrivals,'NA') as r_commodity_arrivals,
    	 									IFNULL(ar.commodity_traded,'NA') as r_commodity_traded,
    	 									
    	 									 IFNULL(ar.Commodity_Uom,'NA') as r_Commodity_Uom,
    	 									IFNULL(ar.created_at,'NA') as r_created_at
