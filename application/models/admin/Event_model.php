...
class Event_model extends CI_Model {
	...
	function event_create($data){
		...
		$log_data['event_id'] = 24;
		$log_data['activity_id'] = $val2['event_id'];
...
//		$this->db->limit(5);
	...
	function event_publish($data){
	 ...
	 //-------------- log table---------------------
        if($data['status'] == 1){
            $this->load->library('lang_file');
            $log_data['event_id'] = 49;
            $log_data['activity_id'] = $data['e_id'];
            $this->lang_file->logg_report($log_data);
        }else{
            $this->load->library('lang_file');
            $log_data['event_id'] = 50;
            $log_data['activity_id'] = $data['e_id'];
            $this->lang_file->logg_report($log_data);
        }
        //----------------------------------------------		
...
//-------------- log table---------------------
		if($data['status1'] == 1){
			$this->load->library('lang_file');
	        $log_data['event_id'] = 51;
	        $log_data['activity_id'] = $data['e_id'];
	        $this->lang_file->logg_report($log_data);
		}else{
			$this->load->library('lang_file');
	        $log_data['event_id'] = 52;
	        $log_data['activity_id'] = $data['e_id'];
	        $this->lang_file->logg_report($log_data);
		}
	...
	function event_update($data){
		...
		//-------------- log table---------------------
		$this->load->library('lang_file');
		$log_data['event_id'] = 25;
		$log_data['activity_id'] = $activity[0]['event_id'];
		$this->lang_file->logg_report($log_data);
		//----------------------------------------------------
