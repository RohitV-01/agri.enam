...
class Links_model extends CI_Model {
	...
	function link_create($data){
		...
		$this->db->trans_begin();
			...
			$log_data['event_id'] = 21;
			$log_data['activity_id'] = $val2['link_id'];
...
//-------------- log table---------------------
				$this->load->library('lang_file');
				$log_data['event_id'] = 22;
				$log_data['activity_id'] = $activity[0]['link_id'];
				$this->lang_file->logg_report($log_data);
				//----------------------------------------------------
	...
	function link_publish($data){
	    ...
	    if($data['status'] == 1){
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 47;
	        $log_data['activity_id'] = $data['l_id'];
	        $this->lang_file->logg_report($log_data);
	    }else{
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 48;
	        $log_data['activity_id'] = $data['l_id'];
	        $this->lang_file->logg_report($log_data);
	    }
	    //----------------------------------------------
