...
class Menu_model extends CI_Model {
	...
	function menu_update($data){
		...
		$this->db->trans_begin();
		$result = $this->db->get_where('menu_item',array('menu_id'=>(int)$data['menu_id'],'lang_id'=>(int)$this->session->userdata('language'),'status'=>1))->result_array();
		if(count($result) > 0){
			...
			//--------------log table---------------------
			$this->db->select('m_id');
			$log_id = $this->db->get_where('menu_item',array('menu_id'=>$data['menu_id'],'lang_id'=>(int)$this->session->userdata('language'),'status'=>1))->result_array();
			
			$this->load->library('lang_file');
			$log_data['event_id'] = 15;
			$log_data['activity_id'] = $log_id[0]['m_id'];
			$this->lang_file->logg_report($log_data);
			//----------------------------------------------------		
		}
		else{
...
//--------------log table---------------------
			$this->load->library('lang_file');
			$log_data['event_id'] = 16;
			$log_data['activity_id'] = $data['m_id'];
			$this->lang_file->logg_report($log_data);
			//-		
		}
		else{
