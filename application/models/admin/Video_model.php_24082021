...
class Video_model extends CI_Model {
	...
	function video_create($data){
		...
		$log_data['event_id'] = 33;
		$log_data['activity_id'] = $val2['video_id'];
	...
	function video_home_page_list(){
	    ...
	    $result = $this->db->query("Select * From (SELECT vi.*,v.sort,v.v_url FROM `video` v
            JOIN video_item vi ON vi.v_id = v.v_id
            WHERE v.publish = 1 AND v.status = 1 AND is_home = 1 AND vi.lang_id = 1 AND v.v_id NOT IN (SELECT v.v_id FROM `video` v
            JOIN video_item vi ON vi.v_id = v.v_id
            WHERE v.publish = 1 AND v.status = 1 AND is_home = 1 AND vi.lang_id = 2)
            UNION
            (SELECT vi.*,v.sort,v.v_url FROM `video` v
            JOIN video_item vi ON vi.v_id = v.v_id
            WHERE v.publish = 1 AND v.status = 1 AND is_home = 1 AND vi.lang_id = 2)) t1 ORDER BY t1.sort ASC")->result_array();
	    return $result;
	}
...
//-------------- log table---------------------
	    if($data['status'] == 1){
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 55;
	        $log_data['activity_id'] = $data['v_id'];
	        $this->lang_file->logg_report($log_data);
	    }else{
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 56;
	        $log_data['activity_id'] = $data['v_id'];
	        $this->lang_file->logg_report($log_data);
	    }
	    //----------------------------------------------
	...
	function video_is_home($data){
	    ...
	    if($data['status1'] == 1){
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 57;
	        $log_data['activity_id'] = $data['v_id'];
	        $this->lang_file->logg_report($log_data);
	    }else{
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 58;
	        $log_data['activity_id'] = $data['v_id'];
	        $this->lang_file->logg_report($log_data);
	    }
	    //----------------------------------------------
	...
	function video_update($data){
		...
		$l_id =	$this->session->userdata('language');
			...
			//-------------- log table---------------------
			$this->load->library('lang_file');
			$log_data['event_id'] = 34;
			$log_data['activity_id'] = $activity[0]['video_id'];
			$this->lang_file->logg_report($log_data);
			//----------------------------------------------------
...
//--------------log table---------------------
		$this->load->library('lang_file');
		$log_data['event_id'] = 9;
		$log_data['activity_id'] = $data['cat_id'];
		$this->lang_file->logg_report($log_data);
		//----------------------------------------------------
