...
//-------------- log table---------------------
	...
	function story_update($data){
		...
		if(count($result) > 0){
			...
			//-------------- log table---------------------
			$this->load->library('lang_file');
			$log_data['event_id'] = 40;
			$log_data['activity_id'] = $result[0]['si_id'];
			$this->lang_file->logg_report($log_data);
			//----------------------------------------------------		
		}
		else{
...
//-------------- log table---------------------
	    if($data['status'] == 1){
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 59;
	        $log_data['activity_id'] = $data['story_id'];
	        $this->lang_file->logg_report($log_data);
	    }else{
	        $this->load->library('lang_file');
	        $log_data['event_id'] = 60;
	        $log_data['activity_id'] = $data['story_id'];
	        $this->lang_file->logg_report($log_data);
	    }
	    //----------------------------------------------
	...
	function story_delete($data){
		...
		$this->db->select('*');
		$activity = $this->db->get_where('success_story_item',array('success_Id'=>$data['story_id']))->result_array();
		
		//-------------- log table---------------------
		$this->load->library('lang_file');
		$log_data['event_id'] = 41;
		$log_data['activity_id'] = $activity[0]['si_id'];
		$this->lang_file->logg_report($log_data);
		//----------------------------------------------------		
		if ($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
