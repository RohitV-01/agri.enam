...
$cat = urldecode($cat);
		...
		if($cat != 'All'){
			...
			if(count($category)>0){
				$this->db->select('vi.*,v.sort,v.v_url,v.v_id as vid,v.publish,v.is_home,v.views');
				$this->db->join('video v','v.v_id=vi.video_id');
				$this->db->order_by('v.sort','ASC');
				$result = $this->db->get_where('video_item vi',array('v.status'=>1,'v.category_id' => $category[0]['v_id'],'vi.lang_id'=>$l_id,'v.publish'=>1,'vi.status'=>1))->result_array();
			...
			$this->db->join('video v','v.v_id=vi.video_id');
			$this->db->order_by('v.sort','ASC');
			$result = $this->db->get_where('video_item vi',array('v.status'=>1,'vi.lang_id'=>$l_id,'v.publish'=>1,'v.publish'=>1,'vi.status'=>1))->result_array();	
		} 
		return $result;
	}
...
//prinr_r($this->db->last_query()); die;
	...
	function video_home_page_list($id){
		...
		$this->db->select('vi.*,v.sort,v.v_url,v.publish,v.is_home,v.views');
		$this->db->join('video v','v.v_id=vi.video_id');
