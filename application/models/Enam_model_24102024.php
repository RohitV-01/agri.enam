...
// 		$result = $this->db->get_where('news n',array('n.status'=>1,'ni.status'=>1,'n.publish'=>1))->result_array();
	...
	function all_menus(){
     ...
     //        WHERE m.status = 1 AND mi.status = 1 AND mi.lang_id = ".$l_id.")) t1 order by t1.id")->result_array();
	    ...
	    $sql = "SELECT mi.*,m.id,m.page_id,m.p_id,m.cms_url,m.external_link FROM `menu` m
            JOIN menu_item mi ON m.id = mi.menu_id
           WHERE m.status = 1 AND mi.status = 1 AND mi.lang_id = ".$l_id." order by m.sort, m.created_at";
	...
	/*for success stories*/
	function getAwardsSuccessStories(){
		$sql = "SELECT si.si_id, si.title, si.success_content, ss.story_image FROM success_story_item si
				INNER JOIN success_story ss on si.success_Id = ss.s_id
				WHERE ss.status = 1 AND ss.publish = 1 and si.story_category = 'awards'";
	...
	function getProcessRelatedSuccessStories(){
		$sql = "SELECT si.si_id, si.title, si.success_content, ss.story_image FROM success_story_item si
				INNER JOIN success_story ss on si.success_Id = ss.s_id
				WHERE ss.status = 1 AND ss.publish = 1 and si.story_category = 'process_related'";
	...
	function getPaymentSuccessStories(){
		$sql = "SELECT si.si_id, si.title, si.success_content, ss.story_image FROM success_story_item si
				INNER JOIN success_story ss on si.success_Id = ss.s_id
				WHERE ss.status = 1 AND ss.publish = 1 and si.story_category = 'payment_weighment'";
	...
	function getResultNotify($sid){
		$sql = "SELECT * FROM notifyDenotify WHERE state_id = '$sid'";
		$result = $this->db->query($sql);
		return $result->result();
	}

	function getStateFromNationalSeed(){
		$sql = "SELECT distinct state FROM national_seeds_corp_info WHERE status = 1";
		$result = $this->db->query($sql);
		return $result->result();
	}

	function getDistFromNationalSeed($state){
		$sql = "SELECT distinct district FROM national_seeds_corp_info WHERE state ='$state' AND status = 1";
		$result = $this->db->query($sql);
		return $result->result();
	}

	function getNationalSeedsDataFromState($stateName, $distName){

		if($stateName != 'Select State' && $distName != 'Select District'){
			$sql = "SELECT * FROM national_seeds_corp_info WHERE state='$stateName' AND district='$distName' AND status = 1";
			$result = $this->db->query($sql);
		}elseif ($stateName != 'Select State' && $distName == 'Select District') {
			$sql = "SELECT * FROM national_seeds_corp_info WHERE state='$stateName' AND status = 1";
			$result = $this->db->query($sql);
		}elseif ($stateName == 'Select State' && $distName == 'Select District') {
			$sql = "SELECT * FROM national_seeds_corp_info WHERE status = 1";
			$result = $this->db->query($sql);
		}
	...
	function getNscDataCount() {
       return $this->db->count_all("national_seeds_corp_info");
   	}
