...
$this->session->set_userdata($newdata);
	...
	public function feedback(){
		...
		$data['main_contant'] = $this->load->view('pages/training_feedback/train_feedback',$data,TRUE);
	...
	//	print_r($_POST);exit;
		if (!empty($_POST)) {
			...
			if (!empty($this->input->post('village'))) {
			...
			if (!empty($this->input->post('feed_state'))) {
				$state = $this->input->post('feed_state');
			} else{
				$state='';
			}
			if (!empty($this->input->post('feed_apmc'))) {
			...
			if (!empty($this->input->post('rating$questionId'))) {
				...
				if ($datafornominee == '') {
					echo "<script>alert('Please get atleast one rating!');window.location.href = 'https://train.enam.gov.in/web/training_feedback';</script>";
				}
		...
		print_r($json_push_data);exit;
			...
			/*  echo "<script>alert('Your Valuable Feedback was Submitted Successfully!');window.location.href = 'https://train.enam.gov.in/web/training_feedback';</script>";*/
