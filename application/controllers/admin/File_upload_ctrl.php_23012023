...
class File_upload_ctrl extends CI_Controller
{
    ...
    // add function for national seed excel import.
    public function importNSC(){
        $path = $this->input->post('path');
        
        if(!empty($_FILES['file']['name'])){
            $file_name = strtotime("now").'_'.$_FILES['file']['name'];
            $temp=$_FILES['file']['tmp_name'];
            if(move_uploaded_file($temp,$path."/".$file_name)){
                echo json_encode(array('msg'=>'Insert Successfully','status'=>'200','filename'=>$file_name));
            }
        }        
    }
