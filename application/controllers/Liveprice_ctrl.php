...
$max_date = $this->db->get_where('mandi_contact_details',array('status'=>1))->result_array();
         ...
         function states_name_live()
        {
			...
			$to_date = $this->input->post('toDate');
        		...
        		$this->db->group_by('min_price','modal_price','max_price');
                $this->db->order_by('state','ASC');
		$result = $this->db->get_Where('curr_day_trade_data',array('Curr_Date'=>$from_date, 'Curr_Date'=>$to_date,'status'=>1))->result_array();
               if(count($result)>0){
			echo json_encode(array('data'=>$result,'status'=>200));
		}
		else{
        ...
        function current_date(){
         // $state_id = $this->input->post('state_id');
		//  $date = $this->db->query("SELECT DISTINCT created_at FROM `curr_day_trade_data` order by created_at DESC limit 1")->result_array();
  ...
  //        if(count($result)>0){
		...
		// }
          ...
          $this->db->order_by('created_at','desc');
          $this->db->limit(1);
          $result = $this->db->get('curr_day_trade_data')->result_array();
          if(count($result)>0){
			echo json_encode(array('data'=>$result,'status'=>200));
		}
		else{
		...
		function trade_data_list(){
            ...
            if($stateName == "-- All --"){
				...
				$sql = "SELECT * from curr_day_trade_data WHERE Curr_Date >= ? AND Curr_Date <= ? group by state,apmc,commodity,min_price,modal_price,max_price,commodity_arrivals,commodity_traded,created_at ORDER BY state ASC, apmc ASC,Curr_Date DESC";
				...
				$result = $this->db->query($sql, array($from_date ,$to_date))->result_array();
					...
					$sql = "SELECT * from curr_day_trade_data WHERE state = ? AND Curr_Date >= ? AND Curr_Date <= ? group by state,apmc,commodity,min_price,modal_price,max_price,commodity_arrivals,commodity_traded,Curr_Date ORDER BY state ASC, apmc ASC,Curr_Date DESC";
		...
		function trade_data_list_1(){
            ...
            if($commodity == "-- All --"){
				...
				$sql = "SELECT * from curr_day_trade_data WHERE Curr_Date >= ? AND Curr_Date <= ? group by state,apmc,commodity,min_price,modal_price,max_price,commodity_arrivals,commodity_traded,Curr_Date ORDER BY state ASC, apmc ASC,Curr_Date DESC";
				...
				$result = $this->db->query($sql, array($from_date ,$to_date))->result_array();
					...
					$sql = "SELECT * from curr_day_trade_data WHERE commodity = ? AND Curr_Date >= ? AND Curr_Date <= ? group by state,apmc,commodity,min_price,modal_price,max_price,commodity_arrivals,commodity_traded,Curr_Date ORDER BY state ASC, apmc ASC,Curr_Date DESC";
