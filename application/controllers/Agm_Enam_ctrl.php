...
class Agm_Enam_ctrl extends CI_Controller {
...
$max_date = $this->db->get_where('mandi_contact_details',array('status'=>1))->result_array();
         ...
         function states_name_live()
        {
        	$from_date = $this->input->post('fromDate');
			$to_date = $this->input->post('toDate');
        		$this->db->select('distinct(ndtd_State)');
        		$this->db->group_by('ndtd_Min_Price','ndtd_Max_Price','ndtd_Modal_Price');
                $this->db->order_by('ndtd_State','ASC');
		$result = $this->db->get_Where('new_dashboard_trade_data',array('ndtd_trn_date'=>$from_date, 'ndtd_trn_date'=>$to_date))->result_array();
               if(count($result)>0){
			echo json_encode(array('data'=>$result,'status'=>200));
		}
		else{
        ...
        function commodity_names()
        {
        	$from_date = $this->input->post('fromDate');
			$to_date = $this->input->post('toDate');
        		$this->db->select('distinct(ndtd_Commodity)');
        		$this->db->group_by('ndtd_Min_Price','ndtd_Max_Price','ndtd_Modal_Price');
                $this->db->order_by('ndtd_Commodity','ASC');
		$result = $this->db->get_Where('new_dashboard_trade_data',array('ndtd_trn_date'=>$from_date,'ndtd_trn_date'=>$to_date))->result_array();
               if(count($result)>0){
			echo json_encode(array('data'=>$result,'status'=>200));
		}
  ...
  //        if(count($result)>0){
		...
		// }
					   ...
					   $this->db->select('DISTINCT(ndtd_trn_date)');
			          $this->db->group_by('ndtd_trn_date');
			          $this->db->order_by('ndtd_trn_date','desc');
			          $this->db->limit(1);
			          $result = $this->db->get('new_dashboard_trade_data')->result_array();
			          if(count($result)>0){
						echo json_encode(array('data'=>$result,'status'=>200));
					}
					else{
		...
		function trade_data_list(){
            ...
            if($stateName == "-- All --"){
				...
				$sql = "SELECT * from new_dashboard_trade_data WHERE ndtd_trn_date >= ? AND ndtd_trn_date <= ? ORDER BY  ndtd_State, ndtd_Mandi, ndtd_Commodity, ndtd_Mandi_type";
				...
				$result = $this->db->query($sql, array($from_date ,$to_date))->result_array();
					...
					$sql = "SELECT * from new_dashboard_trade_data WHERE ndtd_State = ? AND ndtd_trn_date >= ? AND ndtd_trn_date <= ? ORDER BY  ndtd_State, ndtd_Mandi, ndtd_Commodity, ndtd_Mandi_type";
		...
		function trade_data_list_1(){
            ...
            if($commodity == "-- All --"){
				...
				$result = $this->db->query($sql, array($from_date ,$to_date))->result_array();
					...
					$sql = "SELECT * from new_dashboard_trade_data WHERE ndtd_Commodity = ? AND ndtd_trn_date >= ? AND ndtd_trn_date <= ? ORDER BY  ndtd_State, ndtd_Mandi, ndtd_Commodity";
