		...
		<div class="box box-primary">
			<div class="box-header with-border">
			  <h3 class="box-title">All Videos</h3>
				<label>Publish</label>
				<select id="video_publish_drop_down">
					<option value="-1">Select Publish/Un-Publish Video</option>
					<option value="1">Publish</option>
					<option value="0">Un-publish</option>
				</select>
				<label>Is Home</label>
				<select id="is_home_drop_down">
					<option value="-1">Select Is home or Not</option>
					<option value="1">Yes</option>
					<option value="0">No</option>
				</select>
				<label>sort</label>
				<select id="sort_order_drop_down">
					<option value="ASC">ASC.</option>
					<option value="DESC">DESC.</option>
				</select>
				<input type="text" id="video_title_text" placeholder="Enter video title">
				<input type="button" id="drop_down_filter" class="btn btn-default" value="Search">
				<select id="no_pages">
				...
				if(response.status == 200){
					...
					$.each(response.data,function(key,value){
						...
						x = x + '<tr class="">'+
								...
								'<td>';
									if(value.publish == '1'){
										x = x + '<input class="video_published" data-video_id="6" type="checkbox" checked="checked">';
									}
									else { 
										x = x + '<input class="video_published" data-video_id="6" type="checkbox">';
									}
								x = x + '</td>'+
								'<td>';
									if(value.is_home == '1'){
										x = x +'<input class="video_is_home" data-video_id="6" type="checkbox" checked="checked">';
									}
									else {
										x = x +'<input class="video_is_home" data-video_id="6" type="checkbox">';
									}
								x = x + '</td>'+
								'<td>'+
