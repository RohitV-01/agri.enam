...
<div class="col-md-2">
				<label>Sort</label>
					<select class="form-control"  id="news_sort_drop_down">
...
<div class="col-md-2">
					<label>Publish</label>
					<select class="form-control"  id="news_publish_drop_down">
						<option value="-1">News Publish/Un-Publish</option>
...
<div class="col-md-3">
<label>&nbsp;</label>
					<input class="form-control"  type="text" id="news_text_filter" placeholder="Enter news title" />
					</div>
<div class="col-md-3">
<label style="clear:both;width:100%;">&nbsp;</label>
					<input type="button" id="news_filter_search" class="btn btn-info" value="Search" />
...
<label>&nbsp;</label>
				...
				<table class="table">
					...
					<tbody id="news_list_table">
						<?php if(isset($newses) && (count($newses) > 0)){ 
								...
								?>
								<tr class="<?php if(!$find){ echo "find"; } ?> news_highlight news_highlight_<?php echo $news['news_id']; ?>" data-news_id="<?php echo $news['news_id']; ?>">
									...
									<td>
										<?php if($group == 'subadmin'){ ?>
											<a title="Edit" class="btn btn-info btn-flat news_tranlate" data-news_id="<?php echo $news['news_id']?>"><i class="fa fa-edit"></i></a>
										<?php } else { ?>
		...
		<div class="box box-primary">
			...
			<?php if(isset($language)){ ?>
			  	<h3 class="box-title">All News (<?php echo $language['l_name']; ?>)</h3>
			<?php } ?>
			  <div class="box-tools pull-right">
				<button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="Collapse">
				  <i class="fa fa-minus"></i></button>
				...
				<table class="table">
					...
					<tbody>
						<?php if(isset($newses) && (count($newses) > 0)){ 
								...
								<tr class="news_highlight news_highlight_<?php echo $news['news_id']; ?>" data-news_id="<?php echo $news['news_id']; ?>">
									<td><?php echo $news['news_contect']; ?></td>
									<?php if($group != 'subadmin'){ ?>
										<td><?php echo $news['sort']; ?></td>
										<td>
...
<script>
	...
	$(document).on('click','#news_filter_search',function(){
		filter(1);
	});	
	...
	function filter(i = 1){
		var publish = $('#news_publish_drop_down').val();
		var sort = $('#news_sort_drop_down').val();
		var news_text_filter = $('#news_text_filter').val();
		...
		$.ajax({
	        type: 'POST',
	        url: baseUrl+'admin/Ajax_ctrl/news_filter',
	        dataType: "json",
	        data: {
	        	'sort' : sort,
				'publish' : publish,
				'title' : news_text_filter,
				'page' : no_pages
	        },
	        beforeSend: function(){},
	        complete: function(){},
	        success:function (response) {
				if(response.status == 200){
					...
					$.each(response.data,function(key,value){
						x = x + '<tr class="news_highlight news_highlight_'+ value.news_id +'">'+
									'<td>'+ value.news_contect +'</td>';
									...
									x =x + '<td>';
										if($('#u_group').val() != 'subadmin'){
											if(value.publish == "1"){
												x = x + '<input class="news_published" data-news_id="'+ value.news_id +'" type="checkbox" checked="checked">';
											}
											else{
												x = x + '<input class="news_published" data-news_id="'+ value.news_id +'" type="checkbox">';
											}
										}
									x = x +'</td>'+
									'<td>'+
										'<a class="btn btn-info btn-flat news_edit" data-news_id="'+ value.news_id +'"><i class="fa fa-pencil"></i></a> '+
									    '<a class="btn btn-info btn-flat news_delete" data-news_id="'+ value.news_id +'"><i class="fa fa-trash"></i></a>'+
									'</td>'+
								'</tr>';
					});
					$('#news_list_table').html(x);
				}
				else{
					$('#news_list_table').html('No News found on this filter.');
	...
	$(document).on('mouseenter','.news_highlight',function(){
		var id = $(this).data('news_id');
		$('.news_highlight_'+id).addClass('highlight');
	});
	
	$(document).on('mouseleave','.news_highlight',function(){
		var id = $(this).data('news_id');
		$('.news_highlight_'+id).removeClass('highlight');
	});
</script>