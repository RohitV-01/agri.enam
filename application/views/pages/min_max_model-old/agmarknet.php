<?php 
if(isset($param_state) && isset($param_apmc) && isset($param_dist)){ ?>
  <input type="hidden" id="state_id_param" value="<?php echo $param_state; ?>">
  <input type="hidden" id="dist_id_param" value="<?php echo $param_dist; ?>">
...
<form action="<?php echo base_url();?>/Ajax_ctrl/export_trade_details" method="POST">
...
<a href="<?php echo base_url();?>dashboard"><?php echo $this->lang_file->heading_fetch('dashboard');?></a>&nbsp;<i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i>&nbsp;<?php echo $this->lang_file->heading_fetch('agmarknet');?></div>
<?php date_default_timezone_set("Asia/Kolkata");
$date = date("Y-m-d");
?>
<div class="col-md-12" style="padding:0px;"><h3 class="p-title"><span><?php echo $this->lang_file->heading_fetch('agmarknet_price_dashboard');?></span></h3></div>

<div class="col-sm-12 well e-trade-detail-box" >
<input type="hidden" id="previous_date" value="<?php echo date('Y-m-d', strtotime($date .'last day of previous month'));?>">
...
<select class="form-control" id="min_max_state" name="min_max_state">
...
<div class="col-md-2 emandi-select e-trade-inputs1"><b><?php echo $this->lang_file->heading_fetch('min_max_from_date');?></b> <input class="form-control" type="date" id="min_max_apmc_from_date" name="min_max_apmc_from_date" max="<?php echo date('Y-m-d', strtotime($date .' last day of previous month'));?>"/></div>


<div style="padding-left:15px;" class="col-md-2 emandi-select e-trade-inputs1"><b><?php echo $this->lang_file->heading_fetch('min_max_to_date');?></b> <input class="form-control" type="date" id="min_max_apmc_to_date" name="min_max_apmc_to_date" max="<?php echo date('Y-m-d', strtotime($date .' last day of previous month'));?>?>"/></div>

<div style="padding-left:10px;" class="col-md-2 emandi-select e-trade-refresh-b">
	<input style="margin-top:21px;" class="btn btn-refresh" type="button" value="Refresh" id="refresh">
</div>
</div>
<div class="row">
...
<!-- <div class="col-md-12" align="center"><span class="spin" style="display: none;color: black;"> <i class="fa fa-spinner fa-spin"></i></span></div> -->
...
<table class="table table-striped table-bordered">
	<thead>
		<tr>
			...
			<th rowspan="2" style="padding-bottom:2.5%;text-align:center;"><?php echo $this->lang_file->heading_fetch('eNAM_or_Non eNAM');?></th>
			<th rowspan="2" style="padding-bottom:2.5%;text-align:center;"><?php echo $this->lang_file->heading_fetch('min_max_commodity');?></th>
			<th rowspan="2" style="padding-bottom:2.5%;text-align:center;"><?php echo $this->lang_file->heading_fetch('min_max_commo_arrivals');
			?></th>
			<th rowspan="2" style="padding-bottom:2.5%;text-align:center;"><?php echo $this->lang_file->heading_fetch('min_max_commodity_variety');?></th>
			...
			?></th> -->
			<th rowspan="2" style="padding-bottom:2.5%;text-align:center;">
				<?php echo $this->lang_file->heading_fetch('commodity_traded_min_max');?></th>
	...
	<tbody class="tbodya" id="data_list">
		<tr><td colspan="13" style="text-align:center;"><i class="fa fa-spinner fa-spin spin"></i></td></tr>
	</tbody>
</table>
</div>
<!-- <div class="col-md-12" align="right" style="margin-top:21px;">
	<button id="excel_export" type="submit" class="btn btn-info"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export to Excel</button>
</div> -->
</div>
</div>
...
$.ajax({
	type: 'post',
	url: baseUrl+'Agm_ctrl/menu_activate/<?php echo $url_array;?>',
	dataType: "json",
	data:{},
	beforeSend: function(){},
	complete: function(){},
...
$.ajax({
	    type: 'POST',
	    url: baseUrl+'Agm_ctrl/states_name',
	    dataType: "json",
	    data: {},
	    beforeSend: function(){
	    },
		...
		success:function (response) {
			if(response.status == 200){
				...
				$.each(response.data,function(key,value){
					x = x + '<option value="'+ value.AGM_ST_Code +'">'+ value.AGM_Mandi_State_Name +'</option>';
				});
...
function fetch_table_data(){
	...
	var distName = $("#min_max_district option:selected").text();
	...
	$.ajax({
	    type: 'POST',
	    url: baseUrl+'Agm_ctrl/trade_data_list',
	    dataType: "json",
	    data: {
	    	'language' : 'en',
	    	'stateName' : stateName,
		...
		success:function (response) {
			if(response.status == 200){
				...
				$('#data_list').html('<tr><td colspan="13" style="text-align:center;">No record Found.</td></tr>');	
			}
			console.log(response);
		}
	});
...
// console.log(data_array);
	...
	for(var i = parseInt(parseInt(start*limit)+slug); i <= (parseInt(parseInt(parseInt(start)*10))+10); i++){
		if(i < array_length){
			...
			if (data_array[i].Agmp_Non_Enam_Mandi == 0)
			{
				var Non_mandi = "eNAM";
			}
			else
			{
				var Non_mandi = "non eNAM";
			}

			x = x + '<tr>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agmp_State +'</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].AGM_Mandi_District_Name +'</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agmp_Non_Enam_Mandi +'</td>'+
					'<td align="center" style="text-align:center;">'+ Non_mandi +'</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agmp_Commodity +'</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agma_Arrivals +'</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agmp_Variety  +'</td>'+
					'<td align="center" style="text-align:center;">'+ number_formate(data_array[i].Agmp_Min_Price) +'</td>'+
					'<td align="center" style="text-align:center;">'+ number_formate(data_array[i].Agmp_Modal_Price) +'</td>'+
					'<td align="center" style="text-align:center;">'+ number_formate(data_array[i].Agmp_Max_Price) +'</td>'+
					'<td align="center" style="text-align:center;">0</td>'+
					'<td align="center" style="text-align:center;">'+ data_array[i].Agmp_unit +'</td>'+
					'<td align="center" style="text-align:center;">'+ formatDate(data_array[i].Agmp_Arrival_Date) +'</td>';
			x = x + '</tr>';  
		}
		else{
    		break;
...
$(document).on('change','#min_max_state',function(){
	$('#min_max_district').val(0);
	$('#min_max_apmc').val(0);
	$('#min_max_commodity').val(0);
	var AGM_ST_Code = $('#min_max_state').val();
	$.ajax({
		    type: 'POST',
		    url: baseUrl + 'Agm_ctrl/dist_list',
		    dataType: "json",
		    data: {
		    	'AGM_ST_Code' : AGM_ST_Code
		    },
		    beforeSend: function(){
		    ...
		    complete: function(){},
		    success:function (response) {
		    	var x = '<option value="0">-- Select Districts --</option>';
		    	$.each(response.data,function(key,value){
					if($('#dist_id_param').val() == value.AGM_Dist_Code){
						x = x + '<option value="'+ value.AGM_Dist_Code +'" selected>'+ value.AGM_Mandi_District_Name +'</option>';								
					}
					else {
						x = x + '<option value="'+ value.AGM_Dist_Code +'">'+ value.AGM_Mandi_District_Name +'</option>'; 
					}
		    	});
...
$(document).on('change','#min_max_district',function(){
	$('#min_max_apmc').val(0);
	$('#min_max_commodity').val(0);
	var AGM_ST_Code = $('#min_max_state').val();
	var AGM_Dist_Code = $('#min_max_district').val();
	$.ajax({
		    type: 'POST',
		    url: baseUrl + 'Agm_ctrl/apmc_list',
		    dataType: "json",
		    data: {
		    	'AGM_ST_Code' : AGM_ST_Code,
		    	'AGM_Dist_Code' : AGM_Dist_Code
		    },
		    beforeSend: function(){
		    ...
		    success:function (response) {
		    	...
		    	$.each(response.data,function(key,value){
					if($('#apmc_id_param').val() == value.AGM_Mandi_Code){
						x = x + '<option value="'+ value.AGM_Mandi_Code +'" selected>'+ value.AGM_Mandi_Name +'</option>';								
					}
					else {
						x = x + '<option value="'+ value.AGM_Mandi_Code +'">'+ value.AGM_Mandi_Name +'</option>'; 
					}
		    	});
		    	$('#min_max_apmc').html(x);
		    }
...
function commodity_list(){
		...
		var array = {'language' : 'en',
			...
			$.ajax({
	 		    type: 'POST',
	 		    url: baseUrl+'Agm_ctrl/commodity_list',
	 		    dataType: "json",
	 		    data: array,
	 		    beforeSend: function(){	
	 		    },
	 		    ...
	 		    success:function (response) {
	 		    	...
	 		    	$.each(response.data,function(key,value){
	 		    		x = x + '<option value="'+ value.Agmp_Commodity +'">'+ value.Agmp_Commodity +'</option>'; 
					});
