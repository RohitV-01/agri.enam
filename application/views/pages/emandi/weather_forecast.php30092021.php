		...
		<table class="table table-bordered table-striped" id="mandi_table" style="width: 82.5%">
                          ...
                          <div>	<b class="pull-left"><?php
							date_default_timezone_set("Asia/Kolkata");
								echo date("d-m-Y");
							?></b><span class="pull-right"><?php echo $this->lang_file->heading_fetch('weather_forecast_search');?> <b id="table_top_statename_text"></b></span></div>
							</b>
						   </div>
                </table>
		...
		$.ajax({
	        ...
	        success:function (response) {
	        	if(response.status == 200){
					...
					var x = '<thead><tr><th colspan="2"><?php echo $this->lang_file->heading_fetch('weather_forecast_details');?></th></tr></thead>'+
							'<tbody><tr><td style="width:25%;"><b><?php echo $this->lang_file->heading_fetch('weather_forecast_mandi_name');?></b></td><td>'+ response.data[0].wf_APMC+'</td></tr>'+
							'<tr><td><b><?php echo $this->lang_file->heading_fetch('weather_forecast_state_name');?></b></td><td>'+ response.data[0].wf_STATE_UT+'</td></tr>'+
							'<tr><td><b><?php echo $this->lang_file->heading_fetch('weather_forecast_max_temp');?> (&#x2103;)</b></td><td>'+ response.data[0].wf_Max_Temp +'</td></tr>'+
							'<tr><td><b><?php echo $this->lang_file->heading_fetch('weather_forecast_min_temp');?>  (&#x2103;)</b></td><td>'+ response.data[0].wf_Min_temp+'</td></tr>'+
							//'<tr><td><b>Email Address</b></td><td>'+ response.data[0].contact_details+'</td></tr>'+
							'<tr><td><b><?php echo $this->lang_file->heading_fetch('weather_forecast_todays');?></b></td><td>'+ response.data[0].wf_Todays_Forecast +'</td></tr></tbody>';
					console.log(x);
				$('#mandi_table').html(x);	
