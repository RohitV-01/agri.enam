...
<?php $date = date("Y-m-d"); ?>
<?php echo $this->lang_file->heading_fetch('min_max_trading_detail');?>:<!--  <span id="mandi_from_date"></span> to --> <span id="prev_date"></span></b>
...
<input type="hidden" id="cur_date" name="cur_date" value="<?php echo date('Y-m-d', strtotime($date .' -1 day'));?>">
<input type="hidden" id="current_date" value="<?php echo $date;?>">
<div class="col-md-5"></div>
<div class="col-md-2" align="center" style="margin-top: 15px">
...
<div class="pull-left" style="font-size:15px;" id="mandi_content"><b>
<?php $date = date("Y-m-d"); ?>
<?php echo $this->lang_file->heading_fetch('min_max_trading_detail');?>:<!--  <span id="mandi_from_date"></span> to --> <span id="c_date"></span></b>

</div>
<div class="pull-right"><b>Page:</b> <select class="form-control mandi-pagi" name="min_max_no_of_list_1" id="min_max_no_of_list_1"></select></div>
</div>
<div class="col-md-12 table-responsive">	
...
//////////////fetch state////////////////////////////////////
var from_date = $('#previous_date').val();
$.ajax({
	    type: 'POST',
...
function fetch_table_data(){
	...
	$('#prev_date').html(formatDate($('#previous_date').val()));
}
...
function fetch_table_data_1(){
	var commodity_name = $("#min_max_commodity option:selected").text();
	var from_date = $('#previous_date').val();
	$.ajax({
	    type: 'POST',
		...
		success:function (response) {
			if(response.status == 200){
	    		...
	    		var pages = parseInt(parseInt(array_length_1)/parseInt(limit_1));
	    		var y = '';
	    		for(var i = 0;i<= pages; i++){
	        		y = y + '<option value="'+ i +'">'+ parseInt(parseInt(i)+1) +'</option>';
	    		}
	...
	$('#c_date').html(formatDate($('#previous_date').val()));
}
