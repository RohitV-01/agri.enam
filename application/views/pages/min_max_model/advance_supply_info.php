...
<section class="container-fuild content-section emandi-sec" >
	<div class="container">
		<div class="" style="margin-top:10px;">
			...
			<div class="col-md-12"><h3 class="p-title"><?php echo $this->lang_file->heading_fetch('future_crop');?></span></h3></div>
			<div class="col-sm-12 well e-trade-detail-box">
				...
				<div class="col-md-2 emandi-select e-trade-inputs">
					...
					<select class="form-control" id="farmer_district" name="farmer_district" style="width: 161px;">
						<option value="null">-- All --</option>
					</select>
				</div>
				...
				<table class="table table-striped table-bordered" id="future_crop_table">
					<thead>
						<tr>
							...
							<th rowspan="2" style="padding-bottom:2.5%;text-align:center;">Seller Name</th>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on('change','#min_max_state', function(){
			...
			$.ajax({
				url:'<?php echo base_url();?>/Enam_ctrl/getFarmerDistrict',
				method:'POST',
				dataType:'json',
				data:{
					stateVal:stateVal,
				...
				success:function(resDistrict){
					...
					if(resDistrict.status == 'S'){
						for(let data of resDistrict.listData){
							x+=`<option value=${data.farmerDistrictId}>${data.farmerDistrictName}</option>`;
						}
						$('#farmer_district').html(x);
					}
				}
			})
		});

		$(document).on('change','#farmer_district', function(){
			getFarmerCommodity();
			...
			$.ajax({
				url:'<?php echo base_url();?>/Enam_ctrl/getFarmerApmc',
				method:'POST',
				dataType:'json',
				data:{
					stateVal:stateVal,
				...
				success:function(resApmc){
					...
					if(resApmc.status == 'S'){
						for(let data of resApmc.listData){
							x+=`<option value=${data.farmerApmcId}>${data.farmerApmcName}</option>`;
						}
						$('#min_max_apmc').html(x);
					}
				}
		...
		$(document).on('click', '#refresh', function(){
			...
			let toDate = $('#toDate').val();
			let stateVal = $('#min_max_state').val();
			let districtVal = $('#farmer_district').val();
	...
	function getArrvial(stateVal,districtVal,apmcVal,commodityVal,arrivalDays,fromDate, toDate){
		spinner.show();
		$.ajax({
			url:'<?php echo base_url();?>/Enam_ctrl/getFutureCropArrivals',
			method:'POST',
			dataType:'json',
			data:{
				stateVal:stateVal,
	...
	function getFarmerState(){
		...
		$.ajax({
			url:'<?php echo base_url();?>/Enam_ctrl/getFarmerState',
			method:'POST',
			dataType:'json',
			...
			success:function(resState){
				...
				if(resState.status == 'S'){
					for(let data of resState.listData){
						x+=`<option value=${data.farmerStateId}>${data.farmerStateName}</option>`;
					}
	...
	function getFarmerCommodity(){
		...
		$.ajax({
			url:'<?php echo base_url();?>/Enam_ctrl/getFarmerCommodity',
			method:'POST',
			dataType:'json',
			data:{
				stateVal:stateVal,
		...
		for(var i = parseInt(parseInt(start*limit)+slug); i <= (parseInt(parseInt(parseInt(start)*10))+10); i++){
			...
			if(i < array_length){
				...
				x = x + `<tr>
							...
							<td align="center" style="text-align:center;">${data_array[i].farmerStateName}</td>
							<td align="center" style="text-align:center;">${data_array[i].farmerDistrictName == null ? `-` : `${data_array[i].farmerDistrictName}`}</td>
