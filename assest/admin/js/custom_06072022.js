...
//		}
	...
	$(document).on('click','.menu_list_item_delete',function(){
		var m_id = $(this).data('m_id');
		var c = confirm("Are you sure, Delete menu!");
		if(c){
			$.ajax({
				type: 'POST',
		...
		if(form_valid){
			$('#page_add_form').ajaxForm({
			    ...
			    data : { 
					...
					'questions_1' : CKEDITOR.instances.question_1.getData(),
					'ans_1' : CKEDITOR.instances.ans_1.getData(),
					'questions_2' : CKEDITOR.instances.question_2.getData(),
					'ans_2' : CKEDITOR.instances.ans_2.getData(),
					'questions_3' : CKEDITOR.instances.question_3.getData(),
					'ans_3' : CKEDITOR.instances.ans_3.getData(),
					'questions_4' : CKEDITOR.instances.question_4.getData(),
					'ans_4' : CKEDITOR.instances.ans_4.getData(),
					'questions_5' : CKEDITOR.instances.question_5.getData(),
					'ans_5' : CKEDITOR.instances.ans_5.getData(),
					'questions_6' : CKEDITOR.instances.question_6.getData(),
					'ans_6' : CKEDITOR.instances.ans_6.getData(),
					'questions_7' : CKEDITOR.instances.question_7.getData(),
					'ans_7' : CKEDITOR.instances.ans_7.getData(),
					'questions_8' : CKEDITOR.instances.question_8.getData(),
					'ans_8' : CKEDITOR.instances.ans_8.getData()
			    },
			    beforeSubmit:function(e){
					$('#loader').modal('show');
			    },
	...
	$(document).on('keyup','#page_name',function(){
		...
		$.ajax({
	        ...
	        success:function (response) {
	        	console.log(response);
	        	if(response.status == 500){
					$('#page_name').addClass('haserror');
	        		$('#page_create').attr('disabled','disabled');
	        	}
	        	else {
					$('#page_name').removeClass('haserror');
			...
			complete: function(){},
				success:function (response) {
					...
					if(response.status == 200){
						...
						$('#event_category').val(response.data[0].event_category);
						$('#event_update').show();
						$('#event_create').hide();
					}
					else{
...
////////////////////////////////Stories create //////////////////////////////////////
	...
	$(document).on('click','#story_create,#story_update',function(){
		var form_valid = true;
		var story_order = $('#story_order').val();
	...
	/////////////////////////////////////////////////////////////Pop up Notice///////////////////////////////////////////////////////////////////////
	$(document).on('click','#notice_create,#notice_update',function(){
		var form_valid = true;
		var notice_title = $('#notice_title').val();
		var notice_desc = CKEDITOR.instances.notice_desc.getData();
		var notice_order = $('#notice_order').val();

		if(notice_title == ''){
			$('#notice_title_error').html('Notice title is required.').css('display','block');
			form_valid = false;
		}
		else{
			$('#notice_title_error').css('display','none');
		}

		if(notice_desc == ''){
			$('#notice_desc_error').html('notice desc. is Required.').css('display','block');
			form_valid = false;
		}
		else if(notice_desc.length < 17){
			$('#notice_desc_error').html('notice desc. atleast 10 charater.').css('display','block');
			form_valid = false;
		}
		else{ 
			$('#notice_desc_error').css('display','none');
		}
		
		if(notice_order == ''){
			$('#notice_order_error').html('Notice order is required.').css('display','block');
			form_valid = false;
		}
		else{
			$('#notice_order_error').css('display','none');
		}
		if(form_valid){
			$('#pop_form').ajaxForm({
			    dataType : 'json',
			    data : {
			    	...
			    	'notice_desc' : CKEDITOR.instances.notice_desc.getData(),
	...
	$(document).on('click','.notice_edit,.notice_tranlate',function(){
		var pop_id = $(this).data('popup_id');
		$.ajax({
	        type: 'POST',
	        url: baseUrl+'admin/Pop_up_ctrl/get_popup_content',
	        dataType: "json",
	        data: {
	        	'pop_id'	: pop_id
	        },
	        ...
	        success:function (response) {
	        	...
	        	$('#loader').modal('toggle');
	        	if(response.status == 200){
	        		CKEDITOR.instances['notice_desc'].setData(response.data[0].popup_content);
	        		$('#notice_id').val(response.data[0].id);
	        		$('#notice_title').val(response.data[0].title);
	        		$('#notice_order').val(response.data[0].sort);
	        		$('#notice_update').show();
	        		$('#notice_create').hide();
	        	}
	        	else{
	...
	$(document).on('click','.notice_published',function(){
		var x = confirm('Sure to poceed with this!');
		if(!x){
			if($(this).prop('checked') == true){
				$(this).prop('checked', false);
			...
			$.ajax({
		        type: 'POST',
		        url: baseUrl+'admin/Pop_up_ctrl/popup_publish',
		        dataType: "json",
		        data: {
		        	'popup_id'	: popup_id,
		        	'status' : status
	...
	$(document).on('click','.notice_delete',function(){
		...
		if(x){
			...
			$.ajax({
		        type: 'POST',
		        url: baseUrl+'admin/Pop_up_ctrl/popup_delete',
		        dataType: "json",
		        data: {
		        	'popup_id'	: popup_id
		        },
	...
	////////////////////////////////Blogs create //////////////////////////////////////
	$(document).on('click','#blog_create,#blog_update',function(){
	
		var form_valid = true;
		var blog_order = $('#blog_order').val();
		var blog_apmcstate = $('#apmcstate').val();
		var blog_publisher = $('#publisher').val();
		var blog_desc = CKEDITOR.instances.blog_desc.getData();
		//console.log(blog_desc);
		var blog_title = CKEDITOR.instances.blog_title.getData();
		
		if(uGroup != 'subadmin'){
			if($('#blog_order').val()==''){ 
				$('#blog_order_error').html("please Fill blog Sort Order").css('display','block');
				form_valid = false;
			}
			else{
				$('#blog_order_error').css('display','none');
			}
		}

		if(blog_apmcstate == ''){
			$('#apmcstate_error').html("APMC State Error.").css('display','block');
			form_valid = false;
		}else{
			$('#apmcstate_error').css('display','none');
		}
		
		if(blog_desc.length < 10){ 
			$('#blog_desc_error').html("Blog desc.").css('display','block');
			form_valid = false;
		}
		else{
			$('#blog_desc_error').css('display','none');
		}
		
		if(blog_title.length < 10){ 
			$('#story_title_error').html("Story desc.").css('display','block');
			form_valid = false;
		}
		else{
			$('#story_title_error').css('display','none');
		}

		if(blog_publisher == ''){
			$('#publisher_error').html("Please enter publisher.").css('display','block');
			form_valid = false;
		}else{
	...
	$(document).on('click','.blog_edit,.blog_tranlate',function(){ 

		var blog_id = $(this).data('blog_id');
		//alert(blog_id);
		$.ajax({
	        type: 'POST',
	        url: baseUrl+'admin/Blog_ctrl/get_blog_content',
	        dataType: "json",
	        data: {
	        	'blog_id'	: blog_id
	        },
	        ...
	        success:function (response) {
	        	...
	        	if(response.status == 200){
	        		//alert('');
	        		CKEDITOR.instances['blog_title'].setData(response.data[0].title);
	        		CKEDITOR.instances['blog_desc'].setData(response.data[0].blog_content);
	        		$('#blog_id').val(response.data[0].b_id);
	        		$('#blog_order').val(response.data[0].sort);
	        		$('#apmcstate').val(response.data[0].state_apmc);
	        		$('#publisher').val(response.data[0].publisher);
	        		$('#blog_update').show();
	        		$('#blog_create').hide();
	        		$('#loader').modal('toggle');
	        	}
	...
	$(document).on('click','.blog_published',function(){
		var x = confirm('Sure to proceed with this?');
		if(!x){
			if($(this).prop('checked') == true){
				$(this).prop('checked', false);
			...
			$.ajax({
				type: 'POST',
				url: baseUrl+'admin/Blog_ctrl/Blog_publish',
				dataType: "json",
				data: {
					'blog_id'	: blog_id,
					'status' : status
	...
	$(document).on('click','.blog_delete',function(){
		...
		if(x){
			...
			$.ajax({
				type: 'POST',
				url: baseUrl+'admin/Blog_ctrl/blog_delete',
				dataType: "json",
				data: {
					'blog_id'	: blog_id
				},
